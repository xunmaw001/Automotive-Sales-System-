<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.qicheName" type="text" placeholder="汽车名称"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in qicheTypesList" :key="index"
...
<view class="list" :style='{"borderRadius":0,"backgroundColor":"rgba(255, 255, 255, 0)"}'>
    <view style="margin: 10rpx 10rpx 10rpx 10rpx;"
        ...
        <image :style='{"borderRadius":"40rpx","width":"50%","height":"200rpx"}' mode="aspectFill" class="listmpic"
               :src="product.qichePhoto?product.qichePhoto.split(',')[0]:''"></image>
        <view class="listmr">
           ...
           <view style="display: flex;justify-content: space-between;float: left;margin-top: 20%;">
            <text style="margin-left: 50rpx;" v-if="isAuth('qiche','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">
                修改
            </text>
            <text v-if="isAuth('qiche','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('qiche','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 汽车类型 字段 字典表查询方法
		  	let qicheTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'qiche_types'
		  	});
		  	this.qicheTypesList = qicheTypes.data.list;
		...
		methods: {
				...
				let params = {
                    ...
                    ,shangxiaTypes: 1
							...
							if (this.categoryName != 0) {
                                params.qicheTypes = this.categoryName
                            }

                let res = await this.$api.list(`qiche`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('qiche', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.qicheName) {
                    searchForm['qicheName'] = this.searchForm.qicheName
                }
				if (this.searchForm.qicheName) {
					searchForm['qicheName'] = this.searchForm.qicheName
				}

                let res = await this.$api.list(`qiche`, searchForm);
				// 如果是第一页数据置空
