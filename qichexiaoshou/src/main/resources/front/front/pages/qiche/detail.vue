		...
		<view class="container">
			...
			<view class="detail-content">
				<view class="price detail-list-item priceFavor"
					...
					<view class="text" style="color: red;textAlign:left">
						<text style='font-size:20rpx'>¥</text><text style="font-weight:bold;font-size:50rpx">{{detail.qicheNewMoney}}</text><text style="font-size: 20rpx;color: #8799A3;margin-left: 10rpx;">
							<text v-if="detail.qicheOldMoney!=null">原价￥<text style="text-decoration:line-through">{{detail.qicheOldMoney}}</text></text>
                    	</text>
					</view>
				</view>
					<view class="name shop-title"
						  :style='{"boxShadow":"0 0 6rpx rgba(0,0,0,.3)","margin":"0 0 20rpx 0","borderColor":"#fff","backgroundColor":"#fff","color":"#008761","borderRadius":"0","borderWidth":"0","fontSize":"36rpx","borderStyle":"solid"}'>
						汽车名称：{{detail.qicheName}}
					</view>
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.qicheContent"></rich-text>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('qiche', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.qichePhoto ? this.detail.qichePhoto.split(",") : [];
			},
			//添加购物车
            async onCartTap() {
            ...
            onSubmit(){
                uni.setStorageSync('orderGoods', [{
                        qicheId: this.detail.id,
                        qicheName: this.detail.qicheName,
                        qichePhoto: this.swiperList[0],
                    buyNumber: 1,
                        qicheNewMoney: this.detail.qicheNewMoney,
                }])
                this.$utils.jump('../qicheOrder/confirm');
            },
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('qicheCommentback', {
					page: mescroll.num,
					limit: mescroll.size,
					qicheId: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
                this.$utils.jump(`../qicheCommentback/add-or-update?qicheId=${this.id}`)
            },
		}
	}
</script>
