        ...
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        ...
                        v-model='ruleForm.qicheCommentbackText' placeholder="留言内容"></textarea>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`qicheCommentback`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.qicheId){
                this.ruleForm.qicheId = options.qicheId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.qicheCommentbackText)) {
                this.$utils.msg(`留言内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`qicheCommentback`, this.ruleForm);
                } else {
                    await this.$api.save(`qicheCommentback`, this.ruleForm);
                }
